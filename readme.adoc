...
Do you like Java but uses python, groovy, kotlin or similar languages for you scripts ?
...
The `--edit` feature generates a `build.gradle` to use with IDE's that understands it directly + IDE specific config files
to make it easy to run and debug the single file based scripts.
