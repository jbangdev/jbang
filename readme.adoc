...
`jbang --main picocli.codegen.aot.graalvm.ReflectionConfigGenerator info.picocli:picocli-codegen:4.2.0`
...
jbang will by default use `JAVA_HOME` or then `PATH` to locate the `java` executable to run the script with.
...
`//JAVA 13+` will require at least java 13. If Java 13 or higher will be used.
...
To avoid remembering long paths and to enable easy launch of jbang scripts there is a `alias` command
...
|`hello` found in `acme/mycatalog/jbang-catalog.json` in the `dev` branch searched on github, gitlab and bitbucket in tha t order.
