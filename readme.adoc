...
Tested and verified to use on OSX, Linux, Windows and AIX (incl. command.exe, cygwin and mingw shells).

AIX require the GNU `readlink` tool from the GNU coreutils to be available
in the PATH when running `jbang`.  This is not supplied by default with AIX.
Prebuilt vesions can be obtained via the IBM AIX toolbox at 
