...
public class genadoc implements Callable<Integer> {
     ...
     *         <li>2: Usage error: user input for the command was incorrect, e.g.,
     *         the wrong number of arguments, a bad flag, a bad syntax in a
     *         parameter, etc.</li>
     *         <li>4: A template file exists in the template directory. (Remove the
     *         `--template-dir` option or use `--force` to overwrite.)</li>
     *         </ul>
    ...
    static class Config {
        @Option(names = { "-d", "--outdir" }, defaultValue = ".", paramLabel = "<outdir>", description = {
                "Output directory to write the generated AsciiDoc files to. "
                        + "If not specified, files are written to the current directory." })
        File directory;

        @Option(names = { "-t", "--template-dir" }, paramLabel = "<template-dir>", description = {
     ...
     * <li>0: Successful program execution.</li>
     * <li>1: A runtime exception occurred while generating man pages.</li>
     * <li>2: Usage error: user input for the command was incorrect, e.g., the wrong
     * number of arguments, a bad flag, a bad syntax in a parameter, etc.</li>
     * <li>4: A template file exists in the template directory. (Remove the
     * `--template-dir` option or use `--force` to overwrite.)</li>
     * </ul>
     *
    ...
    private static String makeNavFileName(CommandSpec spec) {
        return ("nav-" + spec.qualifiedName("-") + ".adoc").replaceAll("\\s", "_").replace("<main_class>",
                "main_class");
    }
    ...
    static void writeCustomizableManPageTemplate(PrintWriter pw, File includeDir, CommandSpec spec) {
        ...
        List<String> tags = Arrays.asList("header", "name", "synopsis", "description", "options", "arguments",
    ...
    static void genOptions(PrintWriter pw, CommandSpec spec) {
        List<OptionSpec> options = new ArrayList<OptionSpec>(spec.options()); // options are stored in order of
                                                                              // declaration
