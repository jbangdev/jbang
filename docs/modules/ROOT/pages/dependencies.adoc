...
[TIP]
====
For secure authentication `jbang` will honor `~/.m2/settings.xml` and `~/.m2/settings-security.xml` for configuring
username/passwords. Credentials from `~/.netrc`, `GITHUB_TOKEN`, `GITLAB_TOKEN`, and git credential helpers
are also applied automatically — see xref:configuration.adoc#_http_authentication[HTTP Authentication] for details.
====
...
You can use relative paths to reference repositories. This is useful if you want to use a local Maven repository or a custom repository. For example using Gradle and its `publishAllPublicationsToBuildRepository` task, it can be useful to use: `--repos local=./build/repo`.

[WARNING]
====
Relative paths are relative to the current working directory. Thus it will only work if you are using `jbang` from a consistent relative path.  
====
