...
When JBang fetches scripts, catalogs, or other resources over HTTPS, it can authenticate using several credential sources. They are checked in the following order — the first match wins:

1. **`GITHUB_TOKEN`** environment variable — used for `github.com`, `*.github.com`, `githubusercontent.com`, and `*.githubusercontent.com` hosts. Sent as `Authorization: token <value>`.
2. **`GITLAB_TOKEN`** environment variable — used for `gitlab.com` and `*.gitlab.com` hosts. Sent as `Authorization: Bearer <value>`.
3. **URL userinfo** — credentials embedded in the URL itself, e.g. `\https://user:pass@host/path`. Sent as Basic auth.
4. **`.netrc` file** — per-host credentials from `~/.netrc` (Unix/macOS) or `~/_netrc` (Windows). Sent as Basic auth.
5. **`JBANG_AUTH_BASIC_USERNAME` / `JBANG_AUTH_BASIC_PASSWORD`** environment variables — global fallback, applied to all hosts. Sent as Basic auth.
...
By default JBang reads `~/.netrc` (or `~/_netrc` on Windows). To use a different file:
