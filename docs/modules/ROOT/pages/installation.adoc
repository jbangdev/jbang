...
When `JBANG_USE_NATIVE=true`, the wrapper searches for a matching native binary beside the wrapper scripts and then in `./.jbang`. If no local native binary is available, it falls back to the wrapper's `jbang.jar` before using the installation in `$JBANG_DIR/bin`.
...
| `JBANG_USE_NATIVE`
| `false`
| When set to `true`, prefer a platform-specific or generic native binary from the startup script directory or its `.jbang` directory, in that order. Falls back to the corresponding JAR when no native binary is available.
