...
| `JBANG_USE_NATIVE`
| `false`
| When set to `true`, use the native binary (`jbang.bin` / `jbang.bin.exe`) instead of the JAR. If a Unix package installation contains only the JAR, the startup script downloads the matching platform-native binary for the installed JBang version and caches it under `JBANG_CACHE_DIR`.
