...
Brew has a default tap for `jbang` which you can install with:
...
That is perfectly fine for most use cases.

JBang also has a custom tap which is updated at release time of JBang - where as the default tap is updated whenever brew main repo is updated and reviewed:
