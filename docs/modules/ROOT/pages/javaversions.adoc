...
You can also provide an HTTP(S) URL to a JDK archive (`.zip` or `.tar.gz`):

[source,bash]
----
jbang jdk install 17 https://example.org/jdk-17.0.14.zip
----

You can also use local archive files directly, either by path or by `file:` URL:

[source,bash]
----
jbang jdk install 17 /path/to/jdk-17.0.14.tar.gz
jbang jdk install 17 file:///path/to/jdk-17.0.14.zip
----

For archive inputs, JBang unpacks the archive into the managed JDK folder. The installed folder name is `<version>-unknown-jbang`: for numeric `versionOrId` values the version is read from the extracted `release` file, otherwise the provided `versionOrId` is used.
