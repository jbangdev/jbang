...
__JDK_JAVA_OPTIONS__

JBang, being a Java-based application, respects standard Java environment variables. One such variable is `JDK_JAVA_OPTIONS`, which allows you to set default JVM options for all Java applications:
...
Runtime options set via `JDK_JAVA_OPTIONS` applies to all Java applications, including JBang and the app/script that JBang runs.

[source,bash]
----
$ jbang run hello.java
<1>
NOTE: Picked up JDK_JAVA_OPTIONS: -Xmx1g -Dfile.encoding=UTF-8
...
----
<1> JBang process
<2> JBang application process


__JBANG_JAVA_OPTIONS__

Java runtime options intended only for the JBang Java process is set via the `JBANG_JAVA_OPTIONS` environment variable.

[source,bash]
----
# JBang-specific Java runtime options
export JBANG_JAVA_OPTIONS="-XX:TieredStopAtLevel=1"
----

__JBANG_APP_JAVA_OPTIONS__

Java runtime options intended only for the application/script that JBang runs is set via the `JBANG_APP_JAVA_OPTIONS` environment variable.

[source,bash]
----
# JBang App-specific Java runtime options
export JBANG_APP_JAVA_OPTIONS="-Xmx2g"
----
