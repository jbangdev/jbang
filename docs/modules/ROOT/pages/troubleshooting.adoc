...
**Problem**: Slow dependency resolution or transient download failures (especially in CI)
...
4. **Increase download retries**: Set `JBANG_DOWNLOAD_RETRY` to a higher value (default: 5)
+
[source,bash]
----
...
----
+
This applies to JDK and JBang bootstrap downloads on all platforms (curl, wget, and PowerShell).
