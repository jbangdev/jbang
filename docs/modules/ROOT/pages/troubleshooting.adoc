...
5. **UTF-8 console support**: JBang will automatically enable UTF-8 console support by executing command  `chcp.com 65001` and creating environment variable `JBANG_WIN_ENABLE_UTF8` with default value set to `true`. To disable this functionality, set  environment variable `JBANG_WIN_ENABLE_UTF8` to `false` and restart the shell session. `JBANG_WIN_ENABLE_UTF8` can be set to `false` from the command-line using the `setx` command:
+
[source,bash]
----
