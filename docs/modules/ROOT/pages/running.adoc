...
and finally by scanning the JAR for classes with a `main()` method (asking you to pick from the
candidates it finds). Running a scanned candidate automatically requires setting the main class
explicitly. See <<Classpath vs module execution>> for when the descriptor main triggers module
execution.
...
The main class can also use a glob pattern using `?` and `*` to limit the candidates to choose
from. For example:
...
Note how even though there is a single candidate, JBang still asks for confirmation, since you
have not *explicitly* set the main class.
