...
public void shouldInstallWrapper() {
	...
	assertThat(shell(scratch().resolve("jbang").toAbsolutePath() + " echo.java foo")).succeeded().outIsExactly("0:foo\n");
}
...
public void shouldInstallWrapperMissingFolder() {
	assertThat(shell("jbang wrapper install -d foo")).exitedWith(2).errContains("Destination folder does not exist");
}
