...
public abstract class BaseBuilder implements Builder {
	...
	public Project build() throws IOException {
		...
		boolean nativeBuildRequired = prj.isNativeImage() && !Files.exists(prj.getNativeImageFile());
		...
		if (nativeBuildRequired) {
			if (integrationResult.nativeImagePath != null) {
				Files.move(integrationResult.nativeImagePath, prj.getNativeImageFile());
			} else {
				buildNative();
			}
		}
	...
	public void createJar() throws IOException {
		createJar(prj, getBuildDir(), prj.getJarFile());
	}
	...
	private Path getNativeImageOutputName() {
		Path image = prj.getNativeImageFile();
		if (Util.isWindows()) {
