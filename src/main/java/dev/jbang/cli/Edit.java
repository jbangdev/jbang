...
public class Edit extends BaseScriptCommand {
	...
	public Integer doCall() throws IOException {
		...
		File project = createProjectForEdit(dsource, false);
		...
		if (!live) {
			...
			try (final WatchService watchService = FileSystems.getDefault().newWatchService()) {
				File orginalFile = dsource.script().getResourceRef().getFile();
				if (!orginalFile.exists()) {
					throw new ExitException(2, "Cannot live edit " + dsource.getResourceRef().getOriginalResource());
				}
					...
					for (WatchEvent<?> event : wk.pollEvents()) {
						...
						if (Files.isSameFile(orginalFile.toPath(), changed)) {
							try {
								...
								dsource = DecoratedSource.forResource(scriptOrFile);
								createProjectForEdit(dsource, true);
	...
	File createProjectForEdit(DecoratedSource dsource, boolean reload) throws IOException {

		File originalFile = dsource.getResourceRef().getFile();
		...
		if (dsource.getSource() instanceof ScriptSource) {
		...
		// resolveDependencies
		List<MavenRepo> repositories = dsource.script().getAllRepositories();
