...
public class Init extends BaseCommand {
	...
	@Option(name = "ai-provider", description = "AI provider to use")
	String aiProvider;

	@Option(name = "ai-api-key", description = "AI API key")
	String aiApiKey;

	@Option(name = "ai-endpoint", description = "AI endpoint URL")
	String aiEndpoint;

	@Option(name = "ai-model", description = "AI model to use")
	String aiModel;
	...
	public void afterParse() {
		super.afterParse();
		if (aiProvider == null) {
			aiProvider = System.getenv("JBANG_AI_PROVIDER");
		}
		if (aiApiKey == null) {
			aiApiKey = System.getenv("JBANG_AI_API_KEY");
		}
		if (aiEndpoint == null) {
			aiEndpoint = System.getenv("JBANG_AI_ENDPOINT");
		}
		if (aiModel == null) {
			aiModel = System.getenv("JBANG_AI_MODEL");
		}
	}
	...
	public Integer doCall() throws IOException {
		...
		if (!params().isEmpty()) {
			AIProvider provider = new AIProviderFactory(aiProvider, aiApiKey,
					aiEndpoint, aiModel)
				.createProvider();
			if (provider != null) {
				try {
