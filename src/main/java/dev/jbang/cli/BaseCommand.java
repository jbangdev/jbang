...
public abstract class BaseCommand implements Command<CommandInvocation>, CommandLifecycle {
	...
	public CommandResult execute(CommandInvocation commandInvocation) throws InterruptedException {
		...
		try {
			...
			throw new ExitException(EXIT_GENERIC_ERROR, e.getMessage(), e);
		}
	}

	protected static void validateFormat(String format) {
		if (format != null && !"text".equals(format) && !"json".equals(format)) {
			throw new ExitException(EXIT_INVALID_INPUT,
					"Invalid format '" + format + "'. Valid formats: text, json");
		}
	}
