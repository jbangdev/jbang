...
class SearchIntentClassifierTest {
	...
	// --- CLASS intents (prefersCentral = true) ---

	@Test
	void testSimpleClassName() {
		Intent i = SearchIntentClassifier.classify("ObjectMapper");
		assertEquals(IntentType.CLASS, i.type);
		assertTrue(i.prefersCentral());
		assertEquals("ObjectMapper", i.localQuery);
	...
	void testFullyQualifiedClassName() {
		...
		assertEquals(IntentType.CLASS, i.type);
		assertTrue(i.prefersCentral());
		assertEquals("com.fasterxml.jackson.databind.ObjectMapper", i.localQuery);
		assertEquals("fc:com.fasterxml.jackson.databind.ObjectMapper", i.centralQuery);
	}

	@Test
	void testExplicitClassPrefix() {
		Intent i = SearchIntentClassifier.classify("c:StatelessSession");
		assertEquals(IntentType.CLASS, i.type);
		assertTrue(i.prefersCentral());
		assertEquals("StatelessSession", i.localQuery);
		assertEquals("c:StatelessSession", i.centralQuery);
	}
	...
	void testPackageError() {
		...
		assertEquals(IntentType.CLASS, i.type);
		assertTrue(i.prefersCentral());
		assertEquals("org.jsoup", i.localQuery);
	...
	// --- KEYWORD intents (prefersCentral = false) ---
